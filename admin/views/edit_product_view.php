...
ini_set("display_erros", "Off");
    ...
    if(isset($_POST['update_pdt'])){
        $update_msg = $obj->update_product($_POST,$_SESSION['default_lang']); 
        if(!empty($update_msg)){
            $update_msg =$obj->__('product_updated_successfully',$_SESSION['default_lang']);
        }else{
            $update_msg =$obj->__('product_not_updated_successfully',$_SESSION['default_lang']);
        }
    }
    $pdt = $obj->display_productByID($_GET['product_id'],$_SESSION['default_lang']);
...
?>
<h4><?php echo $obj->__('update_product',$_SESSION['default_lang']); ?></h4>
...
<form action="" method="post" enctype="multipart/form-data" class="form">
    ...
    <div class="form-group">
        <label for="pdt_img"><?php echo $obj->__('main_image_of_product',$_SESSION['default_lang']); ?></label>
        <div class="mb-3">
    ...
    <div class="form-group">
        <label for="multiple_images"><?php echo $obj->__('select_multiple_images_product',$_SESSION['default_lang']); ?></label>
    ...
    <div class="form-group">
        ...
        <select name="status" class="form-control">
            <option value="A" <?php if($pdt['status']=='A'){ echo "selected";} ?> ><?php echo $obj->__('active',$_SESSION['default_lang']); ?></option>
            <option value="D" <?php if($pdt['status']=='D'){echo "selected";} ?> ><?php echo $obj->__('disbaled',$_SESSION['default_lang']); ?></option>
        </select>
    </div>

    <input type="submit" value="<?php echo $obj->__('update_product',$_SESSION['default_lang']); ?>" name="update_pdt" class="btn btn-block btn-primary">
</form>
